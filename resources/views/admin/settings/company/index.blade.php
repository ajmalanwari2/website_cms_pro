...
<div class="page-wrapper">
    <div class="page-content">
        ...
        <x-card-header 
            ...
            :permission="'Company - Add New Company'"
            :buttonLabel="$item"
            :buttonClass="'addNewCompanyBtn'"
            :resourcePath="'settings.company'"
            :totalCount="$allCompanies->total()"
...
<script>
    $(document).on('submit', '#add-company-form', function(e) {
        e.preventDefault();
        if(!validateForm('add-company-form')){
            return false;
        }
    ...
    $(document).on('submit', '#edit-company-form', function(e) {
        e.preventDefault();
        if(!validateForm('edit-company-form')){
            return false;
        }
        var formData = new FormData(this);
        $.ajax({
    ...
    let attachmentRowCount = 0; // Counter for attachment row identifiers

    function addNewAttachmentField() {
        attachmentRowCount++;
        const inputFields = `
                <div class="row mt-3" id="attachmentRow_${attachmentRowCount}">
                    <div class="col-md-5">
                        <label for="attachment_name" class="form-label">{{ __('global.company.attachment_name') }} <span class="required">*</span></label>
                        <input type="text" class="form-control" name="r_attachment_name[]" id="attachment_name_${attachmentRowCount}">
                        <span class="text-danger hiddens">{{ __('global.required')}}</span>
                    </div>
                    <div class="col-md-6">
                        <label for="attachment" class="form-label">{{ __('global.company.attachment') }} <span class="required">*</span></label>
                        <input type="file" class="form-control" name="r_attachment[]" id="attachment_${attachmentRowCount}" accept=".jpg, .jpeg, .png, .pdf" >
                        <span class="text-danger hiddens">{{ __('global.required')}}</span>
                    </div>
                    <div class="col-md-1">
                        <label for="attachmentDelete_${attachmentRowCount}" class="form-label">&nbsp;</label>
                        <button type="button" class="btn btn-danger form-control deleteButton" onclick="deleteAttachmentRow(${attachmentRowCount})"><i class="fa fa-trash fs-15"></i></button>
                    </div>
                </div>
            </div>`;

        $('#attachmentArea').append(inputFields);
    }

    function deleteAttachmentRow(rowId) {
        $(`#attachmentRow_${rowId}`).remove(); // Remove the specific row
    }

    function deleteExistingAttachment(attachmentId) {
        // Remove the visible row
        const row = document.getElementById('row_' + attachmentId);
        if (row) {
            row.remove();
        }

        // Add the deleted ID to hidden input for backend
        const deletedInput = document.getElementById('deleted_attachments');
        let deletedIds = deletedInput.value ? deletedInput.value.split(',') : [];

        if (!deletedIds.includes(attachmentId.toString())) {
